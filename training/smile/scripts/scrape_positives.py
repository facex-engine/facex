"""Scrape smile face photos from the web (same strategy as tongue)."""
import os, hashlib, urllib.request, ssl
from pathlib import Path
import cv2
from ddgs import DDGS

OUT = Path("D:/apps/facex/training/data/smile/raw_pos")
OUT.mkdir(parents=True, exist_ok=True)
CROP_OUT = Path("D:/apps/facex/training/data/smile/faces_pos")
CROP_OUT.mkdir(parents=True, exist_ok=True)

YUNET = "D:/apps/facex/_unencrypted_backup/yunet_2023mar.onnx"

QUERIES = [
    "big smile selfie", "wide smile happy face", "laughing face photo",
    "toothy smile portrait", "girl smiling selfie", "man big smile",
    "kid laughing smile", "happy person smiling", "joyful smile face",
    "ear to ear smile", "grinning face photo", "smile teeth photo",
    "candid smile portrait", "wedding smile face", "smile celebrating",
    "huge grin selfie", "smiling face close up", "happy laughing portrait",
    "smiling woman headshot", "smiling man headshot",
]
...
def crop_face(src_path, dst_path, det):
    ...
    cv2.imwrite(str(dst_path), cv2.resize(crop, (112, 112)))
    return True


def main():
    seen = set(); downloaded = 0
    with DDGS() as ddgs:
        for q in QUERIES:
            print(f"\n# {q}")
            try:
            ...
            for r in results:
                ...
                if fetch(url, dst):
                    downloaded += 1
                    if downloaded % 50 == 0: print(f"  saved {downloaded} so far")
            if downloaded >= TARGET_TOTAL: break
