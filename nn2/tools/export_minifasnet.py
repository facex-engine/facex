...
def fold_bn1d_linear(lin, bn1d):
    """Fold BN1d into Linear: y = (W*x+b - μ)/√(v+ε)*γ + β = (γ/√(v+ε))*W*x + (γ/√(v+ε))*(b-μ) + β"""
    W = lin.weight.data.clone()
    if lin.bias is not None:
        b = lin.bias.data.clone()
    else:
        b = torch.zeros(W.shape[0], device=W.device)
    mean = bn1d.running_mean
    var = bn1d.running_var
    gamma = bn1d.weight
    beta = bn1d.bias
    eps = bn1d.eps
    std = torch.sqrt(var + eps)
...
def write_conv_block(f, cb, has_prelu, tag):
    ...
    if has_prelu:
        write_tensor(f, cb.prelu.weight.data.cpu().numpy(), f"{tag}.prelu")
...
def main():
    ...
    with open(args.output, "wb") as f:
        ...
        # prob: 128→3, no bias
        write_tensor(f, net.prob.weight.data.cpu().numpy(), "prob.w")
